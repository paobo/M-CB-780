...
if fskv.get("bauds") == nil then
    ...
    fskv.set("valstate",11) -------  设置阀门状态，1开，2关，3卡住
...
sys.taskInit(function()   ------周期上传mqtt数据
    ...
    while true do
        if mqttc and mqttc:ready() then
            local lll = os.date("!%H:%M:%S",os.time()+28800)  ----- 获得当前整点时间
            local mm_hh = string.sub(lll,1,2)  ----- 获得当前整点时间
            local mm_now = string.sub(lll,4,5)  ----- 获得当前分钟时间
            local ss_now = string.sub(lll,7,8)  ----- 获得当前秒时间
            if tonumber(mm_now) % 60 == 0 then  ----- 如果当前时间是整点
                ---uptime = fskv.get("uptime")
                uptime = 60
            else
                uptime = 60 - tonumber(mm_now)
                ----uptime = (10 - tonumber(string.sub(mm_now,2,2)))*60
            end
            log.info("lll",lll)
    ...
    repeat
        ...
        if #s > 0 then -- #s 是取字符串的长度
            ...
            if string.sub(s:toHex(), 1, 4) == "FEFE" then
                ...
                log.info("ss",ss)
                if string.sub(ss,23,26) == "901F" then
