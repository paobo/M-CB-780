...
sys.taskInit(function()
    ...
    if meterno == nil then ----------------------------如果未能获得表号，说明水表接线或水表硬件故障
        ...
        upCellInfo()
        --if playmode == 2 then
        resetcount = fskv.get("resetcount",resetcount)
        if resetcount == 0 then   -----------------如果是首次上电
...
sys.taskInit(function() ------周期上传mqtt数据
    ...
    while true do
        ...
        if mqttc and mqttc:ready() then
            uart.write(1,
                       string.char(0xFE, 0xFE, 0xFE, 0x68, 0x10, 0xAA, 0xAA,
                                   0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0x01, 0x03,
                                   0x90, 0x1F, 0x01, 0xD2, 0x16))
            ...
            if metersum ~= nil then
                ...
                recharsum = fskv.get("recharsum",recharsum)
                remain = tonumber(recharsum) - tonumber(metersum) ------获得剩余流量
                --end
                local meter_data0 = {
                    DEVTYPE = "M1",
    ...
    --if playmode == 2 then
        ...
        while true do
            ...
            if metersum ~= nil then
                ...
                remain = tonumber(recharsum) - tonumber(metersum) -----获得充值剩余量
...
sys.subscribe("mqtt_payload", function(topic, payload)
    ...
    fls(12)
    if payload ~= nil then
        if string.sub(payload, 1, 2) == "D3" and string.sub(payload, 3, 17) ==
            mobile.imei(0) then
            local bup0 = nil
            local updata2 = nil
            if string.sub(payload, 18, 19) == "C4" then ------ 设置自动上传周期D3867713070630363C40000000060 D3   869020066349869   C4   60(分钟)
                ...
                uptime = string.sub(payload, 20, #payload) --- 使用正则表达式将开头连续的零删除
                ...
                if bup0 ~= "" then
                    if tonumber(bup0) > 0 then -----如果充值数大于0
                        ...
                        if tonumber(recharsum) - tonumber(metersum) >= 0 then ---------- 如果充值满足开阀要求
                            log.info("dddd", tonumber(recharsum))
                            log.info("v3", valstate)
                            if fskv.get("valstate") == 33 or fskv.get("valstate") ==
                                44 then ---------- 如果之前阀门是因欠费而关闭的\或卡住的
            ...
            if string.sub(payload, 18, 19) == "B3" then -------------------清除剩余用量
                sys.timerStart(function()
                    uart.write(1,
                               string.char(0xFE, 0xFE, 0xFE, 0x68, 0x10, 0xAA,
                                           0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA,
                                           0x01, 0x03, 0x90, 0x1F, 0x01, 0xD2,
                                           0x16))
                    if metersum ~= nil then
                        resetcount = 0
