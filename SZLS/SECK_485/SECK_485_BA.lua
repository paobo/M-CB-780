...
local user_name = "kh01"
local password = "kh123456"
local meterno = nil
local alldata = nil
local metersum = ""
local pub_topic = "kh01/s/" .. mobile.imei(0)
local sub_topic = "kh01/p/" .. mobile.imei(0)
---log.info("batt",mybat)
-- if mybat > 4000 then
...
sys.taskInit(function()
    ...
    if ret then -----------------如果gprs网络已经连上ok
        socket.sntp({"time.windows.com","202.120.2.101"})
                ...
                if event == "recv" then -- 如果收到下行数据
                    if payload ~= nil then
                        if string.sub(payload, 1, 2) == "D3" and string.sub(payload, 3, 17) == mobile.imei(0) then
                            ...
                            if string.sub(payload, 18, 19) == "C7" then -- 设置准点上传时间 D3   869020066349869   C7   2355(小时分钟)
                                senttime = string.sub(payload,20,#payload)
                                if isValidTimeFormat(senttime) then
                                    fskv.set("senttime", senttime)
                                    local sentt = fskv.get("senttime")
                                    local re = {
                                        DEVTYPE = "M3",
                                        SN = device_id,
                                        FUNCCODE = "C7",
...
function isValidTimeFormat(timeStr)
    -- 检查输入是否是四个数字，并且符合小时（00-23）和分钟（00-59）的范围
    if string.match(timeStr, "^%d%d%d%d$") then
        local hour = tonumber(timeStr:sub(1, 2))  -- 提取小时
        local minute = tonumber(timeStr:sub(3, 4))  -- 提取分钟
