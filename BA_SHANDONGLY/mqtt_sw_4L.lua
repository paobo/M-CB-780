...
log.info("bauds1",fskv.get("bauds"))
...
sys.taskInit(function()
    ...
    log.info("bauds2", bauds)
    
    --if reason == 2 then
    --    uart.setup(1, bauds, 8, 1, uart.NONE)
    --else
                ...
                if event == "recv" then -- 如果收到下行数据
                    if payload ~= nil then
                        if string.sub(payload, 1, 2) == "D3" and string.sub(payload, 3, 17) == mobile.imei(0) then
                            ...
                            if string.sub(payload, 18, 19) == "C5" then --  开阀或关阀
                                local moto_status0 = nil
                                bup0 = string.sub(payload, 20, #payload)
                                if bup0 == "11" then  ------强制开阀 FE FE FE 68 10 92 00 71 40 02 00 00 2A 04 A0 17 00 55 F7 16
                                    uart.write(1,string.char(0xFE,0xFE,0xFE,0x68,0x10,0x92,0x00,0x71,0x40,0x02,0x00,0x00,0x2A,0x04,0xA0,0x17,0x00,0x55,0xF7,0x16))
                                    local timeout = 12
                                    local startTime = os.clock()
                                    -- while os.clock() - startTime > timeout do    ---电机未开到位  gpio3为行程开关到位指示 Y
                                ...
                                if bup0 ==  "22" then  ------强制关阀 FE FE FE 68 10 92 00 71 40 02 00 00 2A 04 A0 17 00 99 3B 16
                                    uart.write(1,string.char(0xFE,0xFE,0xFE,0x68,0x10,0x92,0x00,0x71,0x40,0x02,0x00,0x00,0x2A,0x04,0xA0,0x17,0x00,0x99,0x3B,0x16))
                                    local timeout0 = 12
...
-----6810891070800000008116901F010 0000000 2C000000002C0000000000000000FF9F16       时代仪表返回格式
-----6810076040020000008109901F010 001000000FF5B16          山东临沂铭信光电表返回格式
    ...
    repeat
        ...
        if #s > 0 then -- #s 是取字符串的长度
            if string.sub(s:toHex(), 1, 4) == "FEFE" then
                ...
                log.info("ss",ss)
                if string.sub(ss,23,26) == "901F" then --6810920071400200008116901F01000100002C010001002C0000000000000001FF5F16
